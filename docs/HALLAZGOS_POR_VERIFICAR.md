...
>   (`claude/stripe-integration-steps-somce4`). HECHA.** Alcance: `clients.py`
>   618/626/702/730/751 (borrado RGPD, "Descargar todo", ZIP de portabilidad),
>   `tests/test_borrado_rgpd.py:74` y `frontend/Caddyfile:43` (el tope de
>   subida contra los vídeos de ejercicio).
...
>   `PortalApp.tsx:290`, `PortalResources.tsx:46`, `portal_public.py` 268/406,
>   `clients.py:927` (contradicciones de la anamnesis) y
>   `feedback_service.py:116` (fotos iniciales como "front").
> - **Tanda 7 (optimización backend + frontend). HECHA.** La hicieron DOS
>   sesiones a la vez (`claude/continue-previous-n6layq` y
>   `claude/google-meet-calendar-integration-1po1c2`), cada una con su
>   implementación; al fusionar se reconcilió hallazgo a hallazgo quedándose
>   con la mejor de cada una. Alcance: `clients.py`
